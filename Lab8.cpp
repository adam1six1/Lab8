...
int main()
{
    ...
    Texture duckTex;
    LoadTex(duckTex, "images/duck3.png");
    ...
    while ((arrows > 0) || drawingArrow) {
        ...
        if (deltaMS > 9) {
            ...
            duckTime += deltaTime.asSeconds();
            if (duckTime > duckSpawn) {
                duckTime = 0;
                    ...
                    duck.setSize(sz);
                    duck.setCenter(Vector2f(sz.x/2, 20 + (sz.y / 2)));
            ...
            scoreText.setPosition(Vector2f(780 - GetTextSize(scoreText).x, 580 - GetTextSize(scoreText).y));
            window.draw(scoreText);
            Text arrowCountText;
            arrowCountText.setString(to_string(arrows));
            arrowCountText.setFont(fnt);
            arrowCountText.setPosition(Vector2f(10, 580 - GetTextSize(scoreText).y));
